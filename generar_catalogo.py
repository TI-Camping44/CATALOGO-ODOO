...
# DICCIONARIO MAESTRO DE LIQUIDACIÓN
...
def main():
    try:
        ...
        for p in products:
            ...
            p['es_liq'] = True if es_liquidacion else False
        ...
        html += f"<li class='nav-item'><button class='btn-filtro' data-filtro='🔥 LIQUIDACIÓN'>🔥 LIQ</button></li>"
        for hoja in orden_hojas:
            if hoja in ["Todo", "🔥 LIQUIDACIÓN"]: continue
            pandas_clone = categorias_datos[hoja]
            if not pandas_clone: continue
        ...
        <script>
            ...
            function renderizarPagina() {
                ...
                items.forEach(p => {
                    ...
                    let badgeLiq = p.l === true ? `<span class='position-absolute top-0 end-0 m-2 badge bg-danger text-white fw-bold shadow' style='font-size:0.8rem; padding:5px 9px; border-radius:8px; z-index:11; letter-spacing:0.5px;'>💥 OFERTA</span>` : '';
                    
                    let preciosHtml = '';
                    for (const [tarifa, precio] of Object.entries(p.p)) {
                        if (stateTarifa !== 'Todas' && tarifa !== stateTarifa) continue;
                        
                        if (p.l === true && OFERTAS_EXACTAS[p.c]) {
                ...
                setTimeout(() => {
                    try {
                        ...
                        productosParaPDF.forEach(p => {
                            ...
                            row.push(""); 
                            row.push(p.c);
                            let nombreFinal = p.l === true ? "[LIQ] " + p.n : p.n;
