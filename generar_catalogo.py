...
def obtener_stock_publicable(models, uid):
    ...
    for g in grupos:
        ...
        stock[pid] = float(g.get('quantity') or 0.0)

    print(f"  Productos con stock publicable: {len(stock)}")
    return stock
...
def main():
    try:
        ...
        for item in pl_items:
            ...
            if not pl_id or pl_id not in pl_name_mapping: continue

            nombre_tarifa = pl_name_mapping[pl_id]
            ...
            if tmpl_id:
                ...
                mapa_precios_tmpl[t_id][nombre_tarifa] = max(precio, mapa_precios_tmpl[t_id].get(nombre_tarifa, 0.0))
        ...
        print("Calculando stock publicable por almacén...")
        stock_publicable = obtener_stock_publicable(models, uid)

        categorias_datos = {}
        orden_hojas = [
            "Todo", "🔥 PROMO", "Municiones", "Armas", "Cargadores", "ASG", "TSS", "CROSMAN", "UMAREX",
        ...
        for p in products:
            ...
            # STOCK: solo almacenes publicables (sin INDGM, NSE, PESS, LIC, IR, SR44)
            stock = float(stock_publicable.get(p['id'], 0.0))
            ...
            if es_arma:
                precio_maygs = mapa_precios_prod.get(prod_id, {}).get("MAYGS")
            ...
            # ===========================================================
            # 30% OFF: SE DEFINE **SOLO** POR LA CATEGORIA DE PRODUCTO
            # Si el categ_id contiene "PROMO" -> va con descuento.
            # ===========================================================
            es_promo = "PROMO" in categoria_str
            ...
            if "CARGADOR" in desc and not any(x in desc for x in ["PORTA", "BASE", "PISO", "ACOPLE"]) and not es_promo:
                hoja = "Cargadores"
            p['stock_calculado'] = stock
            p['hoja_asignada'] = hoja
            p['marca_limpia'] = p['product_brand_id'][1] if p.get('product_brand_id') else "Sin Marca"
            p['es_promo'] = True if es_promo else False

            p_precios = []
            for tarifa in orden_precios:
                p_val = mapa_precios_prod.get(prod_id, {}).get(tarifa)
                if not p_val or p_val <= 0:
                    p_val = mapa_precios_tmpl.get(tmpl_id, {}).get(tarifa, 0.0)
        ...
        for p in categorias_datos["Todo"]:
            ...
            # Si está en PROMO, "Ordenar por precio" usa el precio de OFERTA.
            if p['es_promo']:
                if cod_limpio in PRECIOS_LIQUIDACION_EXACTOS:
                    try:
                        base_price = float(str(PRECIOS_LIQUIDACION_EXACTOS[cod_limpio]).replace(".", "").strip())
                    except (ValueError, AttributeError):
                        pass
                else:
                    base_price = round(base_price * 0.7)
        ...
        total_promo = sum(1 for p in productos_js if p["l"] == True)
        html += f"<li class='nav-item'><button class='btn-filtro active' data-filtro='Todo'>📦 Todo ({len(productos_js)})</button></li>"
        html += f"<li class='nav-item'><button class='btn-filtro' data-filtro='🔥 PROMO'>🔥 PROMO ({total_promo})</button></li>"
        for hoja in orden_hojas:
            if hoja in ["Todo", "🔥 PROMO"]: continue
        ...
        for pl in orden_precios:
            html += f"<div class='form-check form-check-inline m-0'><input class='form-check-input check-tarifa-pdf' type='checkbox' value='{pl}' id='chk_{pl}'><label class='form-check-label small fw-bold text-dark' for='chk_{pl}'>{pl}</label></div>"
        ...
        html += f"<li class='nav-item'><button class='btn-filtro active' data-filtro='Todo'>📦 Todo</button></li>"
        html += f"<li class='nav-item'><button class='btn-filtro' data-filtro='🔥 PROMO'>🔥 PROMO</button></li>"
        for hoja in orden_hojas:
            if hoja in ["Todo", "🔥 PROMO"]: continue
            pandas_clone = categorias_datos[hoja]
            if not pandas_clone: continue
        ...
        <script>
            ...
            // Normaliza texto para buscar SIN acentos (municion == MUNICIÓN).
            const norm = s => (s||"").toString().toUpperCase().normalize("NFD").replace(/[\\u0300-\\u036f]/g,"");

            // Precio PROMO: si el SKU tiene precio fijo cargado, se usa ese;
            // si no, se calcula -30% sobre la tarifa que se está mostrando.
            function formatPromo(sku, precioOriginal) {
            ...
            function aplicarFiltros() {
                ...
                productosFiltrados = PRODUCTOS.filter(p => {
                    ...
                    else if (stateCat === '🔥 PROMO') matchCat = p.l === true;
            ...
            function renderizarPagina() {
                ...
                items.forEach(p => {
                    ...
                    let badgeLiq = p.l === true ? `<span class='position-absolute top-0 end-0 m-2 badge bg-danger text-white fw-bold shadow' style='font-size:0.8rem; padding:5px 9px; border-radius:8px; z-index:11; letter-spacing:0.5px;'>💥 30% OFF</span>` : '';
                    ...
                    for (const [tarifa, precio] of Object.entries(p.p)) {
                        ...
                        if (p.l === true) {
                            let precioOferta = formatPromo(p.c, precio);
                    ...
                    html += `<div class='col-12 col-sm-6 col-md-4 col-lg-4 col-xl-3 tarjeta-contenedor'>
                        <div class='card card-producto shadow-sm d-flex flex-column justify-content-between'>
                            ...
                            <div class='card-body d-flex flex-column justify-content-between p-3 bg-white'>
                                <div class='mb-2'>
                                    <h6 class='fw-bold text-dark text-uppercase mb-1' style='font-size:0.85rem; line-height:1.25; min-height:35px;'>${p.n}</h6>
                ...
                setTimeout(() => {
                    try {
                        ...
                        productosParaPDF.forEach(p => {
                            ...
                            row.push(""); 
                            row.push(p.c);
                            let nombreFinal = p.l === true ? "[PROMO] " + p.n : p.n;
        ...
        print(f"¡Catálogo MAYORISTA generado con éxito! Peso: {peso_final:.2f} MB")
